<?php
$MSG = Array(
    'page_main' => "Головна сторінка",
    'page_dashboard' => "Ваша сторінка",

    'user_current' => "Активний користувач",
    'user_profile' => "Ваш профіль",
    'user_unknown' => "Ви не ввійшли в систему.",
    'user_sign_in' => "Авторизуйтеся",
    'user_sign_or' => "або",
    'user_sign_up' => "зареєструйтеся",
    'user_log_out' => "Вийти з аккаунта.",

    'user' => "Користувач",
    'you' => "Ви",
    'client' => "Замовник",

    'project' => "Проєкт",
    'project_name' => "Назва проєкту",
    'project_desc' => "Опис проєкту",
    'project_type' => "Тип проєкту",
    'project_reward' => "Оплата",
    'project_date' => "Дата",
    'project_completed' => "Завершений",

    'project_post' => "Опублікувати проєкт",
    'projects_open' => "Незавершені проєкти",
    'projects_completed' => "Готові проєкти",
    'projects_none' => "Таких проєктів ще немає",

    'projects_your' => "Проєкти",
    'projects_assigned' => "Доручені проєкти",
    'projects_offered' => "Пропозиції",
    'projects_all' => "Всі незавершені проєкти",
    'project_types' => "Типи проєктів",
    'project_types_all' => "Всі типи проєктів",

    'currency_uah' => "₴",

    'contractor' => "Фрилансер",
    'contractor_assigned' => "Призначений фрілансер",
    'comment' => "Коментарій",
    'comment_date' => "Дата",
    'comment_send' => "Надіслати",

    'action_post' => "Опублікувати",
    'action_edit' => "Редагувати",
    'action_assign' => "Призначити контракт",
    'action_revoke' => "Скасувати призначення",
    'action_finish' => "Закрити проєкт як успішно закінчений",
    'action_delete' => "Видалити",

    'yes' => "Так",
    'no' => "Ні",

    'for_contractors' => "Фрилансерам",
    'for_clients' => "Замовникам",

    'the_login' => "Вхід",
    'the_registration' => "Реєстрація",

    'my_projects' => "Мої завдання",
    'find_project' => "Знайти замовлення",
    'projects' => "Проєкти",
    'contractors' => "Фрилансери",
    'new_project' => "+ Нове замовлення",

    'your_login' => "Ваш логін (E-mail):",
    'your_password' => "Ваш пароль:",
    'your_name' => "Ваше ім'я:",
    'your_surname' => "Ваше прізвище:",
    'your_role' => "Ваша роль:",

    'fill_in' => "Це поле є обов'язковим.",
    'mismatch_lp' => "Надана комбінація паролю і логіну не відповідає жодному користувачеві.",
    'you_are' =>  "",
    'this_is_you' => "Це ви.",
    'mismatch_p' => "Паролі повинні співпадати.",
    'change_p' => "Змінити пароль",
    'new_p' => "Новий пароль:",
    'retype_p' => "Повторіть пароль:",
    'change_i' => "Змінити зображення",
    'change_inf' => "Змінити дані",
    'collision' => "Користувач з таким логіном вже існує.",

    'types' => "Типи проєктів",
    'change_types' => "Зберегти типи проєктів",

    'freelancers_of_type' => "Кількість фрилансерів цієї спеціальності",
    'no_projects' => "Проєктів немає.",
    'no_contractors' => "Фрилансерів немає.",
    'published_by' => "Розміщений користувачем",
    'image_missing' => "Зображення відсутнє.",
);
?>
